...
const lookForDualTrade = async () => {
    const router1 =
        config.routers[Math.floor(Math.random() * config.routers.length)]
            .address;
    const baseAsset =
    ...
    try {
        const returnArray = await arb.instaSearch(
            router1,
            baseAsset,
            tradeSize
        );
...
const dualTrade = async (
    ...
    try {
        ...
        const tx = await arb
            .connect(owner)
            .instaTrade(router1, baseAsset, token2, token3, token4, tradeSize);
        await tx.wait();
        inTrade = false;
        await lookForDualTrade();
    } catch (e) {
